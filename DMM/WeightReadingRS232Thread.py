        ...
        try:
            if 'M' not in response:
                ...
                if weight != self.OLDWEIGHT and self.WSTAT == False:
                    ...
                    if ( ( int(weight) == 0 and self.WCOUNT >= _App.WEIGHT_TRY_ZERO ) or ( int(weight) > 0 and self.WCOUNT >= _App.WEIGHT_TRY_NONZERO ) ) and self.WSTAT == True and ( abs(weight - self.OLDWEIGHT) <= _App.WEIGHT_IGNORE_CHANGES ):
                        self.WSTAT = False
                        self.WCOUNT = 0
